...
async function simulateDeploymentCheck() {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            hasError 
                ? reject(new Error("Perpetual DEX smart contract deployment failed due to insufficient gas.")) 
...
async function checkLiquidity(contractAddress, network) {
    ...
    const spinner = ora(`Verifying liquidity pool for ${contractAddress}...`).start();
    ...
    try {
        ...
        if (parseFloat(balanceInBNB) < 10) {
            throw new Error(`Liquidity pool validation failed: Only ${balanceInBNB} BNB available. Minimum required: 10 BNB`);
        }
...
async function runPerpetualDEXWizard() {
    ...
    const { ownerWallet } = await inquirer.prompt([
        {
            ...
            validate: input => /^0x[a-fA-F0-9]{40}$/.test(input) ? true : "Invalid BSC address format.",
        }
    ]);
    ...
    const { makerFee, takerFee, liquidationFee } = await inquirer.prompt([
        {
            ...
            default: "0.02%",
            validate: input => /^\d+(\.\d+)?%$/.test(input) ? true : "Enter a valid percentage (e.g., 0.02%)",
        },
        {
            type: "input",
            name: "takerFee",
            ...
            default: "0.05%",
            validate: input => /^\d+(\.\d+)?%$/.test(input) ? true : "Enter a valid percentage (e.g., 0.05%)",
        },
        {
            type: "input",
            name: "liquidationFee",
            ...
            default: "1.0%",
            validate: input => /^\d+(\.\d+)?%$/.test(input) ? true : "Enter a valid percentage (e.g., 1.0%)",
        }
    ]);
    ...
    const { tradingPairs } = await inquirer.prompt([
        ...
        "NVDA/USDT",
            ...
            validate: input => input.length >= 1 ? true : "Select at least one trading pair.",
        }
    ]);
    ...
    const { features } = await inquirer.prompt([
        {
            ...
            default: ["Perpetual Futures", "Limit Orders", "Stop-Loss Orders", "Cross Margin"],
        }
    ]);
    ...
    const { maxPositionSize, insuranceFundAllocation } = await inquirer.prompt([
        {
            ...
            default: "1000000",
            validate: input => /^\d+$/.test(input) ? true : "Enter a valid number.",
        },
        {
            type: "input",
            ...
            default: "20%",
            validate: input => /^\d+(\.\d+)?%$/.test(input) ? true : "Enter a valid percentage (e.g., 20%)",
        }
    ]);
    ...
    console.log(chalk.cyan(`   Domain:           `) + chalk.white(`https://${domainName}`));
    console.log(chalk.cyan(`   Contract:         `) + chalk.white(contractAddress));
    console.log(chalk.cyan(`   Owner Wallet:     `) + chalk.white(ownerWallet));
    console.log(chalk.cyan(`   Treasury Wallet:  `) + chalk.white(treasuryWallet));
    console.log(chalk.cyan(`   Max Leverage:     `) + chalk.white(maxLeverage));
    console.log(chalk.cyan(`   Trading Pairs:    `) + chalk.white(tradingPairs.join(", ")));
    ...
    console.log(chalk.cyan(`   2FA Enabled:      `) + chalk.white(enable2FA ? "Yes" : "No"));
